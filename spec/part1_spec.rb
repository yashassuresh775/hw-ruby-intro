...
describe 'Ruby intro part 1' do
  describe '#sum' do
    ...
    it 'returns correct sum', points: 7 do
    ...
    it 'works on the empty array', points: 4 do
  ...
  describe '#max_2_sum' do
    ...
    it 'returns the correct sum', points: 3 do
      expect(max_2_sum([1, 2, 3, 4, 5])).to be_a_kind_of Integer
      expect(max_2_sum([1, -2, -3, -4, -5])).to eq(-1)
    end
    it 'works even if 2 largest values are the same', points: 2 do
      expect(max_2_sum([1, 2, 3, 3])).to eq(6)
    end
    it 'returns zero if array is empty', points: 3 do
      expect(max_2_sum([])).to be_zero
    end
    it 'returns value of the element if just one element', points: 3 do
  ...
  describe '#sum_to_n?' do
    ...
    it 'returns true when any two elements sum to the second argument', points: 9 do
    ...
    it 'returns false for any single element array', points: 1 do
      expect(sum_to_n?([0], 0)).to be false
      expect(sum_to_n?([1], 1)).to be false
      expect(sum_to_n?([-1], -1)).to be false
      expect(sum_to_n?([-3], 0)).to be false
    end
    it 'returns false for an empty array', points: 1 do
